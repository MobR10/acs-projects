...
void initialize(int array[9][9],int row,int column)
{
            k=0;
            array[row][column]=v[k];
}
...
int check3x3(int array[9][9],int row,int column)
{
    for(int i=row-row%3;i<3;i++)
        for(int j=column-column%3;j<3;j++)
            {
                if(array[i][j]==array[row][column]&&row!=i&&column!=j&&array[i][j]!=0)
                    {
                        return 0;
                    }
            }
...
int validate(int array[9][9],int row,int column)
{
    // check row
    for(int j=0;j<9;j++)
        if(array[row][column]==array[row][j]&&column!=j&&array[row][column]!=0)
            return 0;
    // check column
    for(int i=0;i<9;i++)
        if(array[row][column]==array[i][column]&&row!=i&&array[row][column]!=0)
            return 0;
    // check 3x3
    check3x3(array,row,column);
    return 1;
...
void solveSudoku(int array[9][9],int row,int column)
{
        ...
        {
        if(column<8){
            //printf("1 row %d column %d\n",row,column);
            solveSudoku(array,row,column+1);
        }
        else if(row<8){
            //printf("2 row %d column %d\n",row,column);
            solveSudoku(array,row+1,0);
        }
        if(array[row][column]==0)
        {
           progress(array,row,column);
           //de continuat
...
int main() {
    //EXERCITIUL 1
    printf("EXERCITIUL 1\nAfisare primii 30 de termeni din sirul lui Fibonacci\nIterativ: "); fibonacciIterativ(30); printf("\nRecursiv: "); fibonacciRecursive(30); puts("");
    //EXERCITIUL 2
    ...
    int v[5]={1,2,3,4,5};
    displayArrayRecursive(v,5); puts("");
    //EXERCITIUL 4
    /*
